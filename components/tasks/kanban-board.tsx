...
function SortableTask({ task, onClick, onToggle }: { task: any, onClick: () => void, onToggle: (id: string, completed: boolean) => void }) {
 ...
 ref={setNodeRef}
 style={style}
 className="group relative flex flex-col gap-2 rounded-xl border border-border bg-card p-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] transition-all duration-300 ease-in-out cursor-pointer hover:-translate-y-1 hover:shadow-[0_8px_24px_-4px_rgba(var(--section-rgb),0.15)] hover:border-[var(--section-color)]"
 onClick={onClick}
 >
 <div className="flex items-start gap-3">
 {isDraggable && (
 <div {...attributes} {...listeners} className="mt-0.5 cursor-grab text-muted-foreground hover:text-[var(--section-color)] transition-colors">
 ...
 className="mt-0.5 text-muted-foreground hover:text-[var(--section-color)] transition-colors shrink-0"
 >
 <Circle className="h-5 w-5" />
 </button>
 <span className="flex-1 font-bold text-[16px] leading-snug break-all text-foreground">
 {task.title}
 </span>
 </div>
 </div>
...
function CompletedTask({ task, onClick, onToggle }: { task: any, onClick: () => void, onToggle: (id: string, completed: boolean) => void }) {
 return (
 <div
 className="group relative flex flex-col gap-2 rounded-xl border border-border bg-card p-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] transition-all duration-300 ease-in-out cursor-pointer hover:-translate-y-1 hover:shadow-md opacity-70 hover:opacity-100 hover:border-[var(--section-color)]"
 onClick={onClick}
 >
 <div className="flex items-start gap-3">
 ...
 <span className="flex-1 font-bold text-[16px] leading-snug break-all text-muted-foreground line-through">
 {task.title}
 </span>
 </div>
 </div>
...
function SortableColumn({ column, index, tasks, onAddTask, onToggleTask, onTaskClick, onRename, onDelete }: any) {
 ...
 if (isDragging) {
 return <div ref={setNodeRef} style={cssVars} className="flex w-80 flex-shrink-0 flex-col rounded-xl border border-t-[4px] border-border bg-card opacity-50 p-4 h-full" />;
 }

 return (
 <div ref={setNodeRef} style={cssVars} className="group/col flex w-80 flex-shrink-0 flex-col rounded-xl border border-border bg-card border-t-[4px] p-4 h-full shadow-sm transition-all duration-300 ease-in-out hover:shadow-[0_8px_30px_-4px_rgba(var(--section-rgb),0.12)]">
...
export function KanbanBoard({ projectId, projectName, initialColumns, initialTasks }: KanbanBoardProps) {
 ...
 <div className="flex h-16 items-center justify-between border-b px-8 bg-card shrink-0">
 ...
 className="h-9 w-full pl-9 pr-4 rounded-full border-border bg-background focus-visible:ring-primary focus-visible:border-primary text-foreground font-medium transition-all"
 />
 {searchQuery && (
 <button
 type="button"
 ...
 <Button
 variant="outline"
 className="w-full justify-start border-dashed bg-card text-foreground border-border hover:bg-accent hover:text-accent-foreground transition-colors"
 ...
 <form onSubmit={handleAddColumn} className="rounded-xl border border-border bg-card text-foreground p-3 shadow-sm">
 <Input
 autoFocus
 placeholder="Section name"
 value={newColumnName}
 onChange={(e) => setNewColumnName(e.target.value)}
 className="mb-2 h-9 bg-background border-border text-foreground"
 />
 ...
 className="hidden md:inline-flex h-12 w-12 rounded-full shadow-lg border-border bg-card hover:bg-muted shrink-0 disabled:opacity-40"
 >
 <ChevronLeft className="h-6 w-6 text-foreground" />
 </Button>

 <div className="flex-1 bg-card rounded-2xl shadow-2xl border border-border flex flex-col h-[85vh] md:h-[75vh] w-full max-w-md mx-auto overflow-hidden animate-in fade-in zoom-in-95 duration-200">
 <div className="flex items-center justify-between border-b px-4 md:px-6 py-3.5 bg-muted/50">
 <div className="flex items-center gap-2">
 <button
 disabled={focusColumnIndex === 0}
 onClick={() => setFocusColumnIndex(prev => Math.max(0, prev - 1))}
 className="md:hidden p-1 rounded-md hover:bg-muted/50 text-muted-foreground disabled:opacity-30 disabled:hover:bg-transparent transition-colors shrink-0"
 >
 <ChevronLeft className="h-5 w-5" />
 </button>

 <span className="font-bold text-base md:text-lg text-foreground truncate max-w-[120px] sm:max-w-none">
 ...
 <p className="text-sm font-semibold text-muted-foreground">All tasks completed!</p>
 <p className="text-xs text-muted-foreground mt-1">Enjoy the focus space.</p>
 </div>
 ) : (
 filteredTasks
 ...
 className="group flex flex-col gap-2 rounded-xl border border-border bg-background p-4 shadow-sm hover:border-[var(--section-color)] transition-all cursor-pointer"
 >
 <div className="flex items-start gap-3">
 <button
 onClick={(e) => {
 ...
 <span className="font-bold text-[16px] leading-snug break-all text-foreground">
 {task.title}
 </span>
 </div>
 ...
 {task.dueDate && (
 <div className="flex items-center gap-1.5 px-2 py-1 bg-muted rounded-md border border-border text-xs font-medium text-muted-foreground">
 ...
 key={task.id}
 onClick={() => setSelectedTask(task)}
 className="group flex items-start gap-3 rounded-xl border border-border bg-muted p-3 shadow-none opacity-60 hover:opacity-80 transition-all cursor-pointer"
 >
 <button
 onClick={(e) => {
 e.stopPropagation();
 ...
 >
 <CheckCircle2 className="h-5 w-5 text-emerald-500 fill-emerald-50 bg-background rounded-full" />
 </button>
 <span className="font-bold text-[16px] leading-snug break-all text-muted-foreground line-through">
 {task.title}
 </span>
 </div>
 ))}
